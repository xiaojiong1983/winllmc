...
def write_datafile_header(filename, tokslen, model_desc="gpt-2"):
    # saves token data header as a .bin file, for reading in C. add by gjg @2025/9/19
    assert tokslen < 2**31, "token count too large" # ~2.1B tokens
    ...
    # write to file. add by gjg @2025/9/19
    with open(filename, "wb") as f:
        f.write(header.tobytes())

def append_tokens_to_file(filename, tokens, model_desc):
    # appends tokens to an existing .bin file add by gjg @2025/9/19
